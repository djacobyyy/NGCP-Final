...
const osThreadAttr_t payloadControlTask_attributes = {
  .name = "PayloadControlTask",
  .stack_size = 512 * 4,
  .priority = (osPriority_t) osPriorityNormal,
};
/* USER CODE BEGIN PV */
volatile float cmd_pl_angle = 0.0f;
volatile float cmd_plend_angle = 225.0f;
...
void PayloadControlTask(void *argument);
...
int main(void)
{
  ...
  defaultTaskHandle = osThreadNew(NetworkTask, NULL, &defaultTask_attributes);

  /* USER CODE BEGIN RTOS_THREADS */
  osThreadNew(PayloadControlTask, NULL, &payloadControlTask_attributes);
  /* add threads, ... */
		...
		const ip_addr_t *addr, u16_t port)
{
	...
	cmd_pl_angle = data_vals[0];
	cmd_plend_angle = data_vals[1];
	cmd_updated = 1;
}
...
void PayloadControlTask(void *argument)
{
    while (1)
    {
        float angle = cmd_pl_angle;
        float end_angle = cmd_plend_angle;

        if (angle > PL_MAX_ANGLE)
            angle = PL_MAX_ANGLE;

        if (angle < PL_MIN_ANGLE)
            angle = PL_MIN_ANGLE;

        if (end_angle > pl_end.maxLimit)
            end_angle = pl_end.maxLimit;

        if (end_angle < pl_end.minLimit)
            end_angle = pl_end.minLimit;
